...
class MarkdownCodeTester:
    ...
    def __init__(self, merge_blocks: bool = False, timeout_time: int = 300):
        self.merge_blocks = merge_blocks
        self.results: Dict[str, List[Tuple[int, str,
                                           Optional[Exception]]]] = {}
        self.timeout_time = timeout_time  # seconds
    ...
    def test_code_block(self, code: str,
                        block_name: str) -> Optional[Exception]:
        ...
        try:
            ...
            return Exception(
                f"Test timed out after {self.timeout_time} seconds")
            ...
            if passed_blocks:
                ...
                if error is None:
                    results['passed'].append(
                        (passed_blocks[0][0], merged_code[:50] + '...'))
                else:
...
def main():
    ...
    default_timeout_time = 1800  # 30 min
    parser.add_argument(
        '--timeout',
        type=int,
        help='Timeout time to consider for skipping a test (in seconds).'
        'Default: ' + str(default_timeout_time),
