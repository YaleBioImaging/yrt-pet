...
void ArgumentReader::setupCommandLineOptions(cxxopts::Options& options) const
{
	for (const auto& group : mr_registry.getGroups())
	{
		...
		for (auto& [name, arg] : mr_registry.getArguments())
		{
			if (arg.group == group)
			{
				...
				groupOptions(cxxoptsParamName, arg.description, cxxoptsValue);
			}
		}
	}
...
void ArgumentReader::validateRequiredParameters() const
{
	...
	for (const auto& [name, arg] : mr_registry.getArguments())
	{
		if (arg.isRequired)
		{
			...
			if (std::holds_alternative<std::monostate>(argumentValueVariant))
			{
