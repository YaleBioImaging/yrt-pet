 ...
 *   [0,2pi) wrapping); s from the 2D cross-product of LOR endpoints
 * (DOI-correct):
 *
 *   - View angle  phi  = a1,  in [0, pi)  after canonical ordering
 *                        (enforce a1 in [0,pi) by swapping endpoint refs if
 * needed; makes (phi,s) invariant to detector ordering)
 ...
 *
 * Output:
 *   <out>_frame0000.bin  — raw float32, C-order [numAxial, numAngles,
 * numRadial] <out>_frame0001.bin  … <out>_manifest.txt   — one line per frame:
 * "<filename> <startMs> <stopMs>" <out>_geometry.txt   — grid parameters for
 * Python import
 */
...
int main(int argc, char** argv)
{
	try
	{
		...
		registry.registerArgument(
		    "time_step",
		    "Frame duration in milliseconds (default: 1000 = 1 second)", false,
		    io::TypeOfArgument::INT, 1000, framingGroup);

		// ---- Sinogram geometry ----
		registry.registerArgument(
		    "num_axial", "Number of axial midpoint bins (default: 20)", false,
		    io::TypeOfArgument::INT, 20, sinoGroup);
		registry.registerArgument(
		    "num_angles",
		    "Number of view-angle bins over [0, pi) (default: 100)", false,
		    io::TypeOfArgument::INT, 100, sinoGroup);
		registry.registerArgument(
		    "num_radial",
		    "Number of radial offset bins over [0, R] (default: 100)", false,
		    io::TypeOfArgument::INT, 100, sinoGroup);
		...
		std::cout << "Framing window : [" << timeStartArg << ", " << timeStopArg
		          << ") ms (relative to first loaded event)\n"
		          ...
		          << "Bytes per frame: " << sinoSize * sizeof(float) / 1024
		          << " KB\n"
		          << "Total on disk  : "
		...
		const float radialStep =
		    2.0f * R / static_cast<float>(numRadial);  // s in (-R, R)
		...
		std::cout << "Allocating " << numThreads << " per-thread buffers ("
		          << static_cast<double>(totalSize * sizeof(float) *
		                                 numThreads) /
		                 (1024.0 * 1024.0)
		          << " MB total)..." << std::endl;
		...
		util::parallelForChunked(
		    ...
		    [&](size_t evId, size_t threadId)
		    {
			    ...
			    // computeCylindricalCoordinates wraps a1, a2 into [0, 2pi)
			    // correctly.
			    float p1, a1, p2, a2;
			    ScatterSpace::computeCylindricalCoordinates(lor, p1, a1, p2,
			                                                a2);

			    // Canonical ordering: ensure the "first" endpoint is in [0,
			    // pi). This is required to make (phi, s) invariant to detector
			    // ordering. Without it, swapping P1<->P2 leaves phi unchanged
			    // but flips the sign of the cross-product s, so the same LOR
			    // maps to (phi, +s) and (phi, -s) — producing mirrored diagonal
			    // artifacts.
			    ...
			    // phi = a2 could be >= pi and all such events pile up in the
			    // last phi bin, creating a bright artifact band at the top of
			    // the sinogram.
			    const float phi = std::fmod(flip ? a2 : a1, PI_FLT);
			    const float zMid =
			        0.5f * (p1 + p2);  // symmetric, order doesn't matter

			    // Signed radial offset s = (P1 x P2) / |P2xy - P1xy|  in (-R,
			    // R). 2D cross product = signed perpendicular distance from
			    // scanner center to the LOR. Using actual crystal positions
			    // accounts for DOI depth. With canonical ordering enforced
			    // above, s is consistent for both orderings of the same
			    // physical LOR.
			    const float dx = P2.x - P1.x;
			    const float dy = P2.y - P1.y;
			    const float s =
			        (P1.x * P2.y - P1.y * P2.x) / std::sqrt(dx * dx + dy * dy);

			    // ---- Bin indices ----
			    // zMid: shift from [-axialFOV/2, axialFOV/2] to [0, axialFOV]
			    int zBin =
			        static_cast<int>((zMid + 0.5f * axialFOV) / axialStep);
			    ...
			    // No synchronisation needed: each thread writes only to its own
			    // buffer
