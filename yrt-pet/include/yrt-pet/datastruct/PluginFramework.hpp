...
#include "yrt-pet/datastruct/ArgumentDefinition.hpp"
...
public:
	...
	std::vector<std::string>
	    getAllFormats(InputFormatsChoice choice = InputFormatsChoice::ALL) const
	{
	...
	OptionsList
	    getAllOptions(InputFormatsChoice choice = InputFormatsChoice::ALL) const
	{
		OptionsList options;
	...
	{                                                                      \
		...
		{                                                                  \
			plugin::PluginRegistry::instance().registerFormat(             \
			    yrt::util::toUpper(formatName), factoryFunc, optionsAdder, \
...
#define REGISTER_PROJDATA_PYBIND11(formatName, className, moduleAdderFunc)   \
	...
	static className##Pybind11Register global_##className##Pybind11Register; \
	}
