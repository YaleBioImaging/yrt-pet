...
public:
	...
	explicit OperatorVarPsfDevice(const ImageParams& p_imageParams,
	...
	void copyVarPsfToDevice(
	    bool synchronize =
	        true);  // copy kernel LUT to device: allocate and upload
...
protected:
	...
	void varconvolveDevice(const ImageDevice& inputImage,
...
private:
	...
	std::unique_ptr<DeviceArray<int>>
	    mpd_kernelDims;  // triples: kx,ky,kz per kernel
	std::unique_ptr<DeviceArray<int>>
	    mpd_kernelHalfSizes;  // triples: hx,hy,hz per kernel
