...
{
	return ((1ull << (pMSBLimit + 1ull)) - 1ull) - ((1ull << pLSBLimit) - 1ull);
}
             ...
             unsigned int pInsertionLSBLimit, T pToInsert)
{
	...
	T lInsertionMask = generateMask<T>(pInsertionMSBLimit, pInsertionLSBLimit);
	// Bitshift and truncate the insertion
	T lToInsert_shifted = (pToInsert << pInsertionLSBLimit) & lInsertionMask;
...
TDst reinterpretAndCast(void* src, ssize_t offset = 0)
{
