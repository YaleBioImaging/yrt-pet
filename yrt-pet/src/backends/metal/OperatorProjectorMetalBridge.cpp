...
std::size_t estimateCachedBytes(std::size_t eventCount)
{
	return eventCount *
...
bool allocateClearedSiddonImageBuffer(const Context& context,
                                      const Image& image,
                                      Buffer& imageBuffer, float value,
                                      SiddonProjectorKernelProfile* profile)
{
	SiddonForwardImageParams params{};
	if (!context.isValid() ||
	    !makeSiddonForwardImageParams(image, 0, params))
	{
		return false;
	}
	...
	imageBuffer = usePrivateUpdateImageBuffer() ?
	                  Buffer::allocatePrivate(context.device(),
	                      floatByteCount(count)) :
	                  Buffer::allocate(context.device(), floatByteCount(count));
	const bool didClear =
	    imageBuffer.isValid() &&
	    launchProjectionClear(context.device(), context.library(),
	        context.commandQueue(), imageBuffer, value, count);
	if (profile != nullptr)
	{
                      ...
                      bool cacheBuild, double elapsed)
{
	...
	double& cacheBuildTotal =
	    profileAsForward ? profile->forwardGatherCacheBuildSeconds :
	                       profile->adjointGatherCacheBuildSeconds;
	double& uncachedTotal =
	    profileAsForward ? profile->forwardGatherUncachedSeconds :
	                       profile->adjointGatherUncachedSeconds;
	double& directTotal =
	    profileAsForward ? profile->forwardGatherDirectSeconds :
	                       profile->adjointGatherDirectSeconds;
	double& constrainedTotal =
	    profileAsForward ? profile->forwardGatherConstrainedSeconds :
                    ...
                    bool profileAsForward, bool cacheBuild, double elapsed)
{
	...
	double& cacheBuildTotal =
	    profileAsForward ? profile->forwardPackCacheBuildSeconds :
	                       profile->adjointPackCacheBuildSeconds;
	double& uncachedTotal =
	    profileAsForward ? profile->forwardPackUncachedSeconds :
                           ...
                           double elapsed)
{
	...
	double& cacheBuildTotal =
	    profileAsForward ? profile->forwardBatchUploadCacheBuildSeconds :
	                       profile->adjointBatchUploadCacheBuildSeconds;
	double& uncachedTotal =
	    profileAsForward ? profile->forwardBatchUploadUncachedSeconds :
                                      ...
                                      bool& handled)
{
	...
	if (hasMotion)
	{
		...
		{
			motionTransforms[frame] =
			    listMode->getTransformOfMotionFrame(
	...
	util::parallelForChunked(
	    ...
	    [&](std::size_t i, std::size_t /*tid*/)
	    {
		    ...
		    if (hasMotion)
		    {
			    ...
			    if (motionFrame >= 0)
			    {
				    ...
				    const transform_t& transform =
				        motionTransforms[static_cast<std::size_t>(
				            motionFrame)];
		    ...
		    event.bin = bin;
		    event.line =
		        makeCenteredEndpoints(point1, point2, imageParams);
                        ...
                        std::vector<BridgeEvent>& events)
{
	...
	{
		bool handled = false;
		if (!tryGatherListModeLUTBridgeEvents(projectionData, image,
		        binIterator, cache, includeProjectionValues, events, handled))
		{
			return false;
		}
		if (handled)
		...
		util::parallelForChunked(
		    ...
		    [&](std::size_t i, std::size_t /*tid*/)
		    {
			    ...
			    if (!makeBridgeEvent(projectionData, imageParams, bin,
			            projectionData.getLOR(bin), includeProjectionValues,
			            event))
			    {
				    failed.store(true, std::memory_order_relaxed);
				    return;
                            ...
                            OsemRatioBuffers& buffers)
{
	...
	util::parallelForChunked(
	    ...
	    [&](std::size_t i, std::size_t /*tid*/)
	    {
		    ...
		    if (config.hasRandomsEstimates)
		    {
			    randomsValues[i] = corrector.getPrecomputedRandomsEstimate(bin);
		    }
		    if (config.hasScatterEstimates)
		    {
			    scatterValues[i] = corrector.getPrecomputedScatterEstimate(bin);
		    }
		    if (config.hasInVivoAttenuation)
		    {
			    inVivoAttenuationValues[i] =
	...
	buffers.inVivoAttenuation = uploadOptionalFloatVector(context,
	    inVivoAttenuationValues, config.hasInVivoAttenuation, 1.0f);
	if (profile != nullptr)
	{
                           ...
                           const OperatorProjectorMetalOsemConfig& config)
{
	...
	OsemRatioBuffers buffers;
	if (!uploadOsemRatioBuffers(
	        context, profile, measurements, corrector, bins, config, buffers))
	{
		return false;
	}

	const ProjectionOsemRatioParams params{
	    config.globalScaleFactor,
	    ...
	    config.hasInVivoAttenuation ? 1u : 0u};
...
bool computeHostOsemRatioValue(
    const ProjectionData& measurements, const Corrector_CPU& corrector,
    const OperatorProjectorMetalOsemConfig& config, bin_t bin,
    float estimate, float& ratioValue)
{
	if (config.hasSensitivity)
	{
		estimate *= corrector.getPrecomputedSensitivityFactor(bin);
	}
	if (config.hasAttenuation)
	{
		estimate *= corrector.getPrecomputedAttenuationFactor(bin);
	}
	estimate *= config.globalScaleFactor;

	if (config.hasRandomsEstimates)
	{
		estimate += corrector.getPrecomputedRandomsEstimate(bin);
	}
	if (config.hasScatterEstimates)
	{
		estimate += corrector.getPrecomputedScatterEstimate(bin);
	}
	if (config.hasInVivoAttenuation)
	{
		estimate *= corrector.getPrecomputedInVivoAttenuationFactor(bin);
	}
	...
	if (config.hasSensitivity)
	{
		ratioValue *= corrector.getPrecomputedSensitivityFactor(bin);
	}
	if (config.hasAttenuation)
	{
		ratioValue *= corrector.getPrecomputedAttenuationFactor(bin);
	}
	ratioValue *= config.globalScaleFactor;
	return std::abs(ratioValue) != 0.0f;
}

bool computeHostOsemRatioValues(
    OperatorProjectorMetalProfile* profile, const std::vector<bin_t>& bins,
    const std::vector<float>& estimates, const ProjectionData& measurements,
    const Corrector_CPU& corrector,
    const OperatorProjectorMetalOsemConfig& config,
    std::vector<float>& ratioValues, std::size_t& nonzeroCount)
{
    ...
    OperatorProjectorMetalKernel projectorKernel)
{
	...
	for (const auto& [frame, indices] : groupNonNegativeFrames(events))
	{
		...
		const bool didBackProject =
		    batch.isValid() &&
		    makeSiddonForwardImageParams(
		        workingImage, static_cast<std::uint32_t>(frame),
		        imageParams) &&
		    backProjectSingleRay(
		        projectorKernel, context, batch, imageBuffer, imageParams,
		        profile != nullptr ? &kernelProfile : nullptr);
    ...
    const std::vector<BridgeEvent>& events)
{
	...
	for (const auto& [frame, indices] : groupNonNegativeFrames(events))
	{
		...
		addForwardKernelProfile(profile, kernelProfile);
		if (!didForward ||
		    !applyOsemRatioToBatch(
		        context, profile, batch, bins, measurements, corrector, config))
		{
			return false;
		}
		...
		const bool didBackProject =
		    makeSiddonForwardImageParams(
		        ...
		        updateImageParams) &&
		    backProjectSingleRay(
		        config.projectorKernel, context, batch, updateImageBuffer,
		        updateImageParams,
		        profile != nullptr ? &kernelProfile : nullptr);
		if (profile != nullptr)
		{
                        ...
                        CachedIteratorSegment& segment)
{
	...
	segment.byteCount = estimateCachedBytes(events.size());
	segment.cached = true;
	if (events.empty())
	{
		return true;
    ...
    OperatorProjectorMetalKernel projectorKernel)
{
	...
	for (CachedFrameBatch& frameBatch : segment.frameBatches)
	{
		...
		{
			const float projectionValue = projectionData.getProjectionValue(bin);
		...
		const bool didBackProject =
		    makeSiddonForwardImageParams(
		        ...
		        imageParams) &&
		    backProjectSingleRay(
		        projectorKernel, context, frameBatch.batch, imageBuffer,
		        imageParams,
		        profile != nullptr ? &kernelProfile : nullptr);
		addAdjointKernelProfile(profile, kernelProfile);
		if (!didBackProject)
	...
	for (CachedFrameBatch& frameBatch : segment.frameBatches)
	{
		...
		if (!didForward ||
		    !applyOsemRatioToBatch(context, profile, frameBatch.batch,
		        frameBatch.bins, measurements, corrector, config))
		{
			return false;
		}
		...
		const bool didBackProject =
		    makeSiddonForwardImageParams(
		        ...
		        updateImageParams) &&
		    backProjectSingleRay(
		        config.projectorKernel, context, frameBatch.batch,
		        updateImageBuffer,
		        updateImageParams,
		        profile != nullptr ? &kernelProfile : nullptr);
		if (profile != nullptr)
    ...
    const std::vector<BridgeEvent>& events)
{
	...
	for (const auto& [frame, indices] : groupNonNegativeFrames(events))
	{
		...
		const auto downloadStart = Clock::now();
		const bool didDownload =
		    didForward && batch.copyProjectionValuesToHost(estimates);
		if (profile != nullptr)
		{
			profile->forwardDownloadSeconds +=
			    getElapsedSeconds(downloadStart, Clock::now());
		}
		if (!didForward || !didDownload || estimates.size() != bins.size())
		{
			return false;
		}
		...
		if (!computeHostOsemRatioValues(profile, bins, estimates,
		        measurements, corrector, config, ratioValues, nonzeroCount))
		{
			return false;
		}
		if (nonzeroCount == 0)
    ...
    CachedIteratorSegment& segment)
{
	...
	for (CachedFrameBatch& frameBatch : segment.frameBatches)
	{
		...
		if (!didForward || !didDownload ||
		    estimates.size() != frameBatch.bins.size())
		{
			return false;
		}
		...
		std::vector<float> ratioValues;
		std::size_t nonzeroCount = 0;
		if (!computeHostOsemRatioValues(profile, frameBatch.bins,
		        estimates, measurements, corrector, config, ratioValues,
		        nonzeroCount))
		{
			return false;
		}
                         ...
                         std::size_t totalEvents)
{
	...
	return batchLimit == 0 ?
	           totalEvents :
	           std::max<std::size_t>(
	               1, std::min(batchLimit, totalEvents));
}
...
bool buildCachedEntry(const Context& context,
                      ...
                      const BinLoader& binLoader,
                      OperatorProjectorMetalCache& cache,
                      bool profileAsForward, CachedBinIteratorEntry& entry,
                      bool& skippedOverBudget)
{
	        ...
	        std::size_t spanSize, CachedIteratorSegment& segment) -> bool
	{
		...
		return buildCachedSegment(context, profile, events, offset, spanSize,
		                           profileAsForward, segment);
	};

	const std::size_t fullEstimate = estimateCachedBytes(entry.eventCount);
	if (canReserveCacheBytes(cache, 0, fullEstimate))
	{
		CachedIteratorSegment segment;
	...
	for (std::size_t offset = 0; offset < entry.eventCount;
	     offset += batchSize)
	{
		const std::size_t currentBatchSize =
		    std::min(batchSize, entry.eventCount - offset);
		const std::size_t estimatedBytes =
		    estimateCachedBytes(currentBatchSize);
		if (!canReserveCacheBytes(cache, entry.byteCount, estimatedBytes))
		{
...
bool forwardProjectCachedEntry(const Context& context,
                               OperatorProjectorMetalProfile* profile,
                               const Image& image,
                               ...
                               OperatorProjectorMetalKernel projectorKernel)
{
	...
	if (!uploadSiddonImageBuffer(
	        context, image, imageBuffer,
	        profile != nullptr ? &transferProfile : nullptr))
	{
		return false;
	}
	if (profile != nullptr)
                            ...
                            OperatorProjectorMetalKernel projectorKernel)
{
	...
	SiddonProjectorKernelProfile transferProfile;
	if (!uploadSiddonImageBuffer(
	        context, workingImage, imageBuffer,
	        profile != nullptr ? &transferProfile : nullptr))
	{
		return false;
	}
	...
	for (CachedIteratorSegment& segment : entry.segments)
	{
		if (segment.cached)
		{
			if (!backProjectCachedSegmentWithImageBuffer(
			        context, profile, projectionData, workingImage,
			        imageBuffer, segment, projectorKernel))
			{
				return false;
			}
			continue;
	...
	if (!downloadSiddonImageBuffer(
	        context, imageBuffer, workingImage,
	        profile != nullptr ? &transferProfile : nullptr))
	{
		return false;
	}
...
bool applyOsemCachedEntry(const Context& context,
                          OperatorProjectorMetalProfile* profile,
                          const Image& inputImage,
                          const Buffer& inputImageBuffer,
                          Image& workingImage, Buffer& updateImageBuffer,
                          const ProjectionData& measurements,
                          const Corrector_CPU& corrector,
                          const OperatorProjectorMetalOsemConfig& config,
...
void OperatorProjectorMetalCache::setMaxBatchEvents(
    std::size_t maxBatchEvents)
{
...
void OperatorProjectorMetalCache::setMaxChunkEvents(
    std::size_t maxChunkEvents)
{
    ...
    OperatorProjectorMetalKernel projectorKernel) const
{
	...
	auto runUncached = [&]() -> bool
	{
		...
		const std::size_t batchLimit =
		    mp_cache != nullptr ? mp_cache->maxBatchEvents()
		                        : kDefaultMaxBatchEvents;
		const std::size_t batchSize =
		    batchLimit == 0 ? totalEvents : std::max<std::size_t>(
	...
	if (entryIt != mp_cache->mp_impl->entries.end())
	{
		...
		return forwardProjectCachedEntry(m_context, mp_profile, image,
		                                 projectionData, binIterator,
		                                 binLoader, mp_cache,
		                                 entryIt->second, projectorKernel);
	}
	...
	if (!buildCachedEntry(m_context, mp_profile, projectionData, image,
	        binIterator, binLoader, *mp_cache, true, entry,
	        skippedOverBudget))
	{
		return runUncached();
	}
	...
	return forwardProjectCachedEntry(m_context, mp_profile, image,
	                                 projectionData, binIterator, binLoader,
	                                 mp_cache, insertedIt->second,
	                                 projectorKernel);
}
    ...
    OperatorProjectorMetalKernel projectorKernel) const
{
	...
	auto runUncached = [&]() -> bool
	{
		...
		SiddonProjectorKernelProfile transferProfile;
		if (!uploadSiddonImageBuffer(
		        m_context, workingImage, imageBuffer,
		        mp_profile != nullptr ? &transferProfile : nullptr))
		{
			return false;
		}
	...
	bool skippedOverBudget = false;
	if (!buildCachedEntry(m_context, mp_profile, projectionData, image,
	        binIterator, binLoader, *mp_cache, false, entry,
	        skippedOverBudget))
	{
		return runUncached();
	}
    ...
    const OperatorProjectorMetalOsemConfig& config) const
{
	...
	SiddonProjectorKernelProfile transferProfile;
	if (!uploadSiddonImageBuffer(
	        m_context, inputImage, inputImageBuffer,
	        mp_profile != nullptr ? &transferProfile : nullptr))
	{
		return false;
	}
	...
	const bool didInitializeUpdateImageBuffer =
	    config.zeroInitializeUpdateImage ?
	        allocateClearedSiddonImageBuffer(
	            ...
	            mp_profile != nullptr ? &transferProfile : nullptr) :
	        uploadSiddonImageBuffer(
	            m_context, workingImage, updateImageBuffer,
	            mp_profile != nullptr ? &transferProfile : nullptr);
	...
	if (entryIt != mp_cache->mp_impl->entries.end())
	{
		...
		if (!applyOsemHostRatioCachedEntry(m_context, mp_profile,
		        inputImage, inputImageBuffer, workingImage,
		        updateImageBuffer, measurements, corrector, config,
		        binIterator, binLoader, mp_cache, entryIt->second))
		{
			return false;
		}
		return finish();
	...
	CachedBinIteratorEntry entry;
	bool skippedOverBudget = false;
	if (!buildCachedEntry(m_context, mp_profile, measurements, inputImage,
	        binIterator, binLoader, *mp_cache, true, entry,
	        skippedOverBudget))
	{
		return runUncached();
	...
	if (!applyOsemHostRatioCachedEntry(m_context, mp_profile, inputImage,
	        inputImageBuffer, workingImage, updateImageBuffer, measurements,
	        corrector, config, binIterator, binLoader, mp_cache,
	        insertedIt->second))
	{
		return false;
	}
    ...
    const OperatorProjectorMetalOsemConfig& config) const
{
	...
	if (entryIt != mp_cache->mp_impl->entries.end())
	{
		...
		if (!applyOsemCachedEntry(m_context, mp_profile, inputImage,
		        inputImageBuffer, workingImage, updateImageBuffer,
		        measurements, corrector, config, binIterator, binLoader,
		        mp_cache, entryIt->second))
		{
			return false;
		}
		return finish();
