...
void py_setup_constraints(py::module& m)
{
	...
	c_DetectorMask.def(py::init<const DetectorMask*>(), py::arg("detmask"));
}
}  // namespace yrt
...
ConstraintDetectorMask::ConstraintDetectorMask(const DetectorMask* mask)
{
	m_constraintFcn = [mask](const ConstraintManager& manager,
	                         const PropertyUnit* info, size_t pos)
	{
		const det_id_t d1 = manager.getDataValue<det_id_t>(
		    info, pos, ConstraintVariableType::DET1);
		const det_id_t d2 = manager.getDataValue<det_id_t>(
		    info, pos, ConstraintVariableType::DET2);
		return (mask->isDetectorEnabled(d1) && mask->isDetectorEnabled(d2));
	};
}
