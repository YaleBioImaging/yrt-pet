    ...
    pybind11::array_t<double> kappa_list, int num_threads)
{
	...
	{
		std::stringstream err;
		err << "A matrix should have shape [K, T] " << "([" << num_kappa << ", "
		    << num_frames << "])";
		throw std::runtime_error(err.str());
	}
	if (buf_B_all.shape[0] != num_kappa || buf_B_all.shape[1] != num_k)
	{
		std::stringstream err;
		err << "B matrix should have shape [K, P] " << "([" << num_kappa << ", "
		    << num_k << "])";
		throw std::runtime_error(err.str());
	}
	...
	{
		std::stringstream err;
		err << "A_n_inv matrix should have shape [K] " << "([" << num_kappa
		    << "])";
		throw std::runtime_error(err.str());
	}
	if (buf_W.size != num_frames)
...
void solveSRTM2Basis(const T* tac_all, T* kin_out, const T* kin_p,
                     ...
                     const T* W, const T* Lambda, const T alpha, const T k2p,
                     ...
                     const int num_threads)
{
	...
	util::parallelForChunked(
	    ...
	    [tac_buff_ptr, tac_all, num_frames, num_pix, num_kappa, A_all, B_all,
	     ...
	     kin_out](size_t pi, size_t tid)
	    {
		    // Fill buffer
		    T* tac_buff_t = tac_buff_ptr + tid * num_frames;
		    ...
		    for (int ki = 0; ki < num_kappa; ki++)
		    {
			    ...
			    {
				    y += alpha *
				         (B[0] * Lambda[0] * (kin_p[0 * num_pix + pi] + 1) +
			    ...
			    {
				    T k_diff_0 =
				        kin_p[0 * num_pix + pi] - (theta * k2p / kappa - 1);
	     ...
	     kin_out](size_t pi, size_t tid)
	    {
		    ...
		    for (int ki = 0; ki < num_kappa; ki++)
		    {
			    ...
			    if (alpha > 0.f && kin_p != nullptr)
			    {
				    ...
				    y1 += alpha *
				          (B[1] * Lambda[0] * (kin_p[0 * num_pix + pi] + 1) +
                         ...
                         const int num_threads)
{
	...
	util::parallelForChunked(
	    ...
	    {
		    // Fill buffer
		    T* tac_buff_t0 = tac_buff_ptr + tid * 2 * num_frames;
		    T* tac_buff_t1 = tac_buff_t0 + num_frames;
		    for (int ti = 0; ti < num_frames; ti++)
		    {
			    tac_buff_t0[ti] = tac_all[ti * num_pix + pi];
			    tac_buff_t1[ti] =
			        tac_all[num_pix * num_frames + ti * num_pix + pi];
		    }
		    T cost_min = std::numeric_limits<T>::max();
		    T opt_bp_b = -1.f;
		    T opt_k2_b = -1.f;
		    ...
		    for (int ki = 0; ki < num_kappa; ki++)
		    {
			    ...
			    if (alpha > 0.f && kin_p != nullptr)
			    {
				    ...
				    y2 += alpha *
				          (B[2] * Lambda[0] * (kin_p[0 * num_pix + pi] + 1) +
				    ...
				    y3 += alpha *
				          (B[3] * Lambda[0] * (kin_p[0 * num_pix + pi] + 1) +
			    ...
			    // Calculate inverse
			    T theta_0 = Rinv_Qt[0] * y0 + Rinv_Qt[1] * y1 +
			                Rinv_Qt[2] * y2 + Rinv_Qt[3] * y3;
			    T theta_1 = Rinv_Qt[4] * y0 + Rinv_Qt[5] * y1 +
			                Rinv_Qt[6] * y2 + Rinv_Qt[7] * y3;
			    T theta_2 = Rinv_Qt[8] * y0 + Rinv_Qt[9] * y1 +
			                Rinv_Qt[10] * y2 + Rinv_Qt[11] * y3;
			    T theta_3 = Rinv_Qt[12] * y0 + Rinv_Qt[13] * y1 +
			                Rinv_Qt[14] * y2 + Rinv_Qt[15] * y3;
			    ...
			    for (int ti = 0; ti < num_frames; ti++)
			    {
				    ...
				    T res1 = tac_buff_t1[ti] - (A1[ti * 4 + 2] * theta_2 +
				                                A1[ti * 4 + 3] * theta_3);
