...
void OperatorProjector::applyA(const Variable* in, Variable* out)
{
	...
	if (mp_projector->getUpdaterType() == UpdaterType::DEFAULT4D)
	{
		ASSERT_MSG(
		    util::doesDynamicFramingMatch(*dat, img->getParams()),
    ...
    OperatorProjector::getProjectionPropertyTypes() const
{
	...
	// It is impossible to know yet, and from the OperatorProjector alone, if
	...
	//  a dynamic framing or not. Therefore, we always gather it in case it
	//  is needed.
