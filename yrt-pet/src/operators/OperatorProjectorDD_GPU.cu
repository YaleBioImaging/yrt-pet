	...
	if (projPsfDevicePointer == nullptr)
	{
		if (tofHelperDevicePointer == nullptr)
		{
			launchKernel<IsForward, false, false>(
			    ...
			    projPropManager, nullptr /*No TOF*/,
			    nullptr
			    /*No ProjPSF*/,
		...
		else
		{
			launchKernel<IsForward, true, false>(
			    dat.getProjValuesDevicePointer(), img.getDevicePointer(),
			    pd_updater, dat.getProjectionPropertiesDevicePointer(),
			    projPropManager, tofHelperDevicePointer,
			    nullptr
			    /*No ProjPSF*/,
			    {} /*No ProjPSF*/, cuScannerParams, cuImageParams,
			    getBatchSize(), getGridSize(), getBlockSize(), getMainStream(),
	...
	else
	{
		...
		if (tofHelperDevicePointer == nullptr)
		{
			launchKernel<IsForward, false, true>(
			    dat.getProjValuesDevicePointer(), img.getDevicePointer(),
			    pd_updater, dat.getProjectionPropertiesDevicePointer(),
			    projPropManager, nullptr /*No TOF*/, projPsfDevicePointer,
		...
		else
		{
			launchKernel<IsForward, true, true>(
			    dat.getProjValuesDevicePointer(), img.getDevicePointer(),
			    pd_updater, dat.getProjectionPropertiesDevicePointer(),
			    projPropManager, tofHelperDevicePointer, projPsfDevicePointer,
			    projectionPsfProperties, cuScannerParams, cuImageParams,
			    getBatchSize(), getGridSize(), getBlockSize(), getMainStream(),
			    synchronize);
		}
    ...
    bool synchronize)
{
	...
	if (stream != nullptr)
	{
		DD_kernel<IsForward, HasTOF, HasProjPsf>
		    <<<gridSize, blockSize, 0, *stream>>>(
		        pd_projValues, pd_image, pd_updater, pd_projProperties,
		        pd_projPropManager, pd_tofHelper, pd_projPsfKernels,
		        projectionPsfProperties, scannerParams, imgParams, batchSize);
		if (synchronize)
		{
	...
	{
		DD_kernel<IsForward, HasTOF, HasProjPsf><<<gridSize, blockSize>>>(
		    pd_projValues, pd_image, pd_updater, pd_projProperties,
		    pd_projPropManager, pd_tofHelper, pd_projPsfKernels,
		    projectionPsfProperties, scannerParams, imgParams, batchSize);
		if (synchronize)
                          ...
                          CUImageParams imgParams, size_t batchSize)
{
	...
	if (eventId < batchSize)
	{
		...
		float3 d1{lor[0], lor[1], lor[2]};
		float3 d2{lor[3], lor[4], lor[5]};
		...
		const float3 d2_minus_d1 = d1_minus_d2 * (-1.0f);
					...
					for (int zi = dd_z_i_0; zi <= dd_z_i_1; zi++)
					{
						...
						if (dd_z_r_1 >= dd_z_p_0 && dd_z_r_0 < dd_z_p_1)
						{
							...
							if constexpr (IsForward)
							{
								// value += (*ptr) * weight;
								value += (*pd_updater)
								    ->forwardUpdate(weight, pd_image, idx,
								                    dynamicFrame,
								                    numVoxelsPerFrame);
							}
							else
							{
								...
								(*pd_updater)
								    ->backUpdate(value, weight, pd_image, idx,
								                 dynamicFrame,
								                 numVoxelsPerFrame);
							}
						}
...
template __global__ void DD_kernel<false, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<false, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<false, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<false, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<true, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<true, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<true, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
    ProjectionPsfProperties projectionPsfProperties,
    CUScannerParams scannerParams, CUImageParams imgParams, size_t batchSize);
template __global__ void DD_kernel<true, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, const float* pd_projPsfKernels,
