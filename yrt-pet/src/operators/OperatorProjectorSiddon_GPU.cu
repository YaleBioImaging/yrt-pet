                                                     ...
                                                     bool synchronize)
{
	...
	if (tofHelperDevicePointer == nullptr)
	{
		OperatorProjectorSiddon_GPU::launchKernel<IsForward, false>(
		    ...
		    pd_updater, dat.getProjectionPropertiesDevicePointer(),
		    projPropManager, nullptr /*No TOF*/, cuScannerParams, cuImageParams,
	...
	else
	{
		OperatorProjectorSiddon_GPU::launchKernel<IsForward, true>(
		    dat.getProjValuesDevicePointer(), img.getDevicePointer(),
		    pd_updater, dat.getProjectionPropertiesDevicePointer(),
		    projPropManager, tofHelperDevicePointer, cuScannerParams,
    ...
    unsigned int blockSize, const cudaStream_t* stream, bool synchronize)
{
	...
	else
	{
		if (stream != nullptr)
		{
			Siddon_kernel<IsForward, HasTOF, true, true>
			    <<<gridSize, blockSize, 0, *stream>>>(
			        pd_projValues, pd_image, pd_updater, pd_projProperties,
			        pd_projPropManager, pd_tofHelper, scannerParams, imgParams,
			        m_numRays, batchSize);
...
__device__ curandState setupMultiRays(unsigned long seed, unsigned long id,
                                      ...
                                      float3& parallelToTrans1,
                                      float3& parallelToTrans2)
{
	curandState state;
    ...
    CUImageParams imgParams, int p_numRays, size_t batchSize)
{
	...
	if (eventId < batchSize)
	{
		...
		const float3 p1_minus_p2 = p1_init - p2_init;
		float tofValue = 0.0f;
		if constexpr (HasTOF)
		{
		...
		for (int i_line = 0; i_line < numRays; i_line++)
		{
			...
			while (a_cur < amax && !flag_done)
			{
				...
				if constexpr (IsForward)
				{
					value +=
					    (*pd_updater)
					        ->forwardUpdate(weight, raw_img_ptr, totalOffset,
					                        dynamicFrame, numVoxelsPerFrame);
				}
				else
				{
					(*pd_updater)
					    ->backUpdate(value, weight, raw_img_ptr, totalOffset,
					                 dynamicFrame, numVoxelsPerFrame);
				}
				a_cur = a_next;
				ax_next_prev = ax_next;
...
template __global__ void Siddon_kernel<true, true, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, true, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, false, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, false, false, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, true, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, true, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, false, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, false, true, false>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, true, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, true, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, false, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, false, false, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, true, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, true, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<true, false, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
    CUImageParams imgParams, int p_numRays, size_t batchSize);
template __global__ void Siddon_kernel<false, false, true, true>(
    float* pd_projValues, float* pd_image, UpdaterPointer pd_updater,
    const char* pd_projProperties,
    const ProjectionPropertyManager* pd_projPropManager,
    const TimeOfFlightHelper* pd_tofHelper, CUScannerParams scannerParams,
