...
{
	auto c = py::class_<OperatorProjectorUpdater,
	                    std::shared_ptr<OperatorProjectorUpdater>>(
	    m, "OperatorProjectorUpdater");
	c.def(
	    "forwardUpdate",
	    [](OperatorProjectorUpdater& self, float weight, Image* in_image,
	       int offset_x, frame_t dynamicFrame, size_t numVoxelPerFrame) -> float
	    {
	    ...
	    py::arg("dynamicFrame") = 0, py::arg("numVoxelPerFrame") = 0);
	c.def(
	    "backUpdate",
	    [](OperatorProjectorUpdater& self, float value, float weight,
	       Image* in_image, int offset_x, frame_t dynamicFrame,
	       size_t numVoxelPerFrame) -> void
	    {
		    self.backUpdate(value, weight, in_image->getRawPointer(), offset_x,
		                    dynamicFrame, numVoxelPerFrame);
	    },
	...
	// Default updater
	auto def_upd =
	    py::class_<OperatorProjectorUpdaterDefault3D, OperatorProjectorUpdater,
	               std::shared_ptr<OperatorProjectorUpdaterDefault3D>>(
	        m, "OperatorProjectorUpdaterDefault3D");
	def_upd.def(py::init<>());

	auto def_upd_4d =
	    py::class_<OperatorProjectorUpdaterDefault4D, OperatorProjectorUpdater,
	...
	// LR updater
	auto lr_upd =
	    py::class_<OperatorProjectorUpdaterLR, OperatorProjectorUpdater,
	...
	lr_upd.def("setUpdateH", [](OperatorProjectorUpdaterLR& self, bool updateH)
...
float OperatorProjectorUpdaterDefault3D::forwardUpdate(
    float weight, float* cur_img_ptr, size_t offset, frame_t dynamicFrame,
    ...
    frame_t dynamicFrame, size_t numVoxelPerFrame, int tid)
{
	...
	std::atomic_ref<float> atomic_elem(
	    cur_img_ptr[dynamicFrame * numVoxelPerFrame + offset]);
	atomic_elem.fetch_add(output);
}
...
void OperatorProjectorUpdaterLR::setHBasisWrite(
    const Array2DBase<float>& pr_HWrite)
{
...
const Array3D<double>& OperatorProjectorUpdaterLR::getHBasisWriteThread()
{
...
{
	int numThreads = m_HWriteThread.getDims()[0];
