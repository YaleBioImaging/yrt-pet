...
void py_setup_operatorvarpsfdevice(py::module& m)
{
	...
	c.def("copyVarPsfToDevice", &OperatorVarPsfDevice::copyVarPsfToDevice,
		  ...
		  "Upload the PSF kernels to the GPU");
	c.def("applyA",[](OperatorVarPsfDevice& self, const Image* img_in, Image* img_out)
...
void OperatorVarPsfDevice::copyVarPsfToDevice(bool synchronize)
{
    ...
    for (size_t k = 0; k < nKernels; ++k)
    {
        ...
        // Flatten in z‑major order consistent with Array3D layout
        ...
        currentOffset += static_cast<size_t>(sx) * static_cast<size_t>(sy) * static_cast<size_t>(sz);
    }
                                 ...
                                 bool synchronize) const
{
    ...
    if (img_in != nullptr)
    {
        ...
        inputImageDevice = std::make_unique<ImageDeviceOwned>(img_in->getParams(), getMainStream());
    ...
    if (img_out != nullptr)
    {
        ...
        outputImageDevice = std::make_unique<ImageDeviceOwned>(img_out->getParams(), getMainStream());
        reinterpret_cast<ImageDeviceOwned*>(outputImageDevice.get())->allocate();
    ...
    {
        varconvolveDevice<false>(*inputImageDevice_ptr, *outputImageDevice_ptr, false);
    }
    else
    {
        varconvolveDevice<true>(*inputImageDevice_ptr, *outputImageDevice_ptr, false);
    }
...
template void OperatorVarPsfDevice::apply<false>(const Variable* in, Variable* out,
												bool synchronize) const;
template void OperatorVarPsfDevice::apply<true>(const Variable* in, Variable* out,
											   bool synchronize) const;
                                             ...
                                             bool synchronize) const
{
	...
	if (stream != nullptr)
	{
		if constexpr (IS_FWD)
		{
			convolve3D_kernel_F<<<launchParams.gridSize, launchParams.blockSize, 0, *stream>>>(pd_input, pd_output,
				pd_kernelsFlat, pd_kernelOffsets, pd_kernelDims, pd_kernelHalf,
				lut_x_dim, lut_y_dim, lut_z_dim,
				d_xGap, d_yGap, d_zGap, xCenter, yCenter, zCenter,
				vx, vy, vz, nx, ny, nz);
			if (synchronize)
			{
				cudaStreamSynchronize(*stream);
		...
		{
			convolve3D_kernel_T<<<launchParams.gridSize, launchParams.blockSize, 0, *stream>>>(pd_input, pd_output,
				pd_kernelsFlat, pd_kernelOffsets, pd_kernelDims, pd_kernelHalf,
				lut_x_dim, lut_y_dim, lut_z_dim,
				d_xGap, d_yGap, d_zGap, xCenter, yCenter, zCenter,
				vx, vy, vz, nx, ny, nz);
	...
	else
	{
		if constexpr (IS_FWD)
		{
			convolve3D_kernel_F<<<launchParams.gridSize, launchParams.blockSize, 0>>>(pd_input, pd_output,
				pd_kernelsFlat, pd_kernelOffsets, pd_kernelDims, pd_kernelHalf,
				lut_x_dim, lut_y_dim, lut_z_dim,
				d_xGap, d_yGap, d_zGap, xCenter, yCenter, zCenter,
				vx, vy, vz, nx, ny, nz);
		...
		{
			convolve3D_kernel_T<<<launchParams.gridSize, launchParams.blockSize, 0>>>(pd_input, pd_output,
				pd_kernelsFlat, pd_kernelOffsets, pd_kernelDims, pd_kernelHalf,
				lut_x_dim, lut_y_dim, lut_z_dim,
				d_xGap, d_yGap, d_zGap, xCenter, yCenter, zCenter,
				vx, vy, vz, nx, ny, nz);
...
template void OperatorVarPsfDevice::varconvolveDevice<true>(
