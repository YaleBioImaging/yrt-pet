...
void declare_array_base(py::module& m, const std::string& dtype_name)
{
	...
	c.def("getDims", &T::getDims);
	c.def("getShape", &T::getDims);
...
void declare_array_child(py::module& m, const std::string& dtype_name)
{
	...
	if constexpr (T::IsOwned)
	{
		...
		c.def("allocate",
		      [](T& self, const std::array<size_t, T::NDim>& dims)
		      {
			      ASSERT_MSG(dims.size() == T::NDim,
			                 "The shape given has the "
			                 "incorrect number of dimensions");

			      // YN: Yes, this code is ugly, but it provides convenience.
			      if constexpr (T::NDim == 1)
			      {
				      self.allocate(dims[0]);
			      }
			      else if constexpr (T::NDim == 2)
			      {
				      self.allocate(dims[0], dims[1]);
			      }
			      else if constexpr (T::NDim == 3)
			      {
				      self.allocate(dims[0], dims[1], dims[2]);
			      }
			      else if constexpr (T::NDim == 4)
			      {
				      self.allocate(dims[0], dims[1], dims[2], dims[3]);
			      }
			      else if constexpr (T::NDim == 5)
			      {
				      self.allocate(dims[0], dims[1], dims[2], dims[3],
				                    dims[4]);
			      }
