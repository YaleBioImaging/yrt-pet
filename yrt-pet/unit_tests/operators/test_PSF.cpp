...
void runVarPsf2GTest(std::default_random_engine& engine,
                     const std::string& suffix)
{
	for (int i = 0; i < 100; ++i)
	{
		...
		SECTION("forward_varpsf_2g" + suffix)
		{
		...
		SECTION("transpose_varpsf_2g" + suffix)
		{
		...
		SECTION("adjoint_test_varpsf_2g" + suffix)
		{
...
TEST_CASE("VarPSF_2G", "[varpsf_2g]")
{
	const unsigned int randomSeed =
	    static_cast<unsigned int>(std::time(nullptr));
	std::default_random_engine engine(randomSeed);
	runVarPsf2GTest<OperatorVarPsf>(engine, "");
}
#if BUILD_CUDA
...
TEST_CASE("VarPSF_2G_GPU", "[varpsf_2g_gpu]")
{
	const unsigned int randomSeed =
	    static_cast<unsigned int>(std::time(nullptr));
	std::default_random_engine engine(randomSeed);
	runVarPsf2GTest<OperatorVarPsfDevice>(engine, "_gpu");
}
#endif  // BUILD_CUDA
}  // namespace yrt::util::test
