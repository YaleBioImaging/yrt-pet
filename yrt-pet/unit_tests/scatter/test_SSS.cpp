...
{
	constexpr size_t maxVRAMToUse = (2ull << 20);  // 2 Mb
	...
	const float lengthX = scanner->scannerRadius * yrt::SQRT2_FLT;
	const float lengthY = scanner->scannerRadius * yrt::SQRT2_FLT;
	const float lengthZ = scanner->axialFOV;
	...
	yrt::scatter::SingleScatterSimulator sss(*scanner, *mu, *lambda);

	constexpr size_t nTOF = 1;  // TODO: Change this once SSS supports TOF
	constexpr size_t nPlanes = 8;
	constexpr size_t nAngles = 30;
	...
	SECTION("non-direct planes")
	{
		...
		sss.runSSS(cpuOut, false);
		sss.runSSSDevice(gpuOut, false, maxVRAMToUse);
	...
	SECTION("direct planes only")
	{
		...
		sss.runSSSDevice(gpuOut, true, maxVRAMToUse);
