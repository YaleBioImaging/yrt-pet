...
TEST_CASE("scatterspace", "[scatterspace]")
{
	...
	{
		auto scanner = yrt::util::test::makeFakeScanner();
		yrt::ScatterSpace space(*scanner, 2, 2, 4);
		space.allocate();
	...
	SECTION("edgecases")
	{
		...
		// minimum-num-bins
		{
			yrt::ScatterSpace space(*scanner, 1, 1, 4);
			// Need at least 4 angles

			// Should still work without crashing
			REQUIRE(space.getNumTOFBins() == 1);
			REQUIRE(space.getNumPlanes() == 1);
			REQUIRE(space.getNumAngles() == 4);
	...
	SECTION("random-grid-sizes")
	{
		...
		std::uniform_int_distribution<> sizeDistribution(4, 50);
	...
	{
		auto scanner = yrt::util::test::makeFakeScanner();
		yrt::ScatterSpace space(*scanner, 3, 3, 4);
		space.allocate();
	...
	// TODO: Add unit test for usage of getProjectionValueFromHistogramBin
	// TODO: Add test for reading and writing files
}
