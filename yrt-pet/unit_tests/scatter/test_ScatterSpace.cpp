...
TEST_CASE("scatterspace-construction", "[scatterspace]")
{
...
TEST_CASE("scatterspace-properties", "[scatterspace]")
{
	...
	SECTION("radius-diameter")
	{
		REQUIRE(space.getRadius() == Approx(300.0f));
		REQUIRE(space.getDiameter() == Approx(600.0f));
	}

	SECTION("axialfov")
	{
		REQUIRE(space.getAxialFOV() == Approx(400.0f));
	}

	SECTION("maxtof")
	{
		// "scanner diameter"/"speed of light"
		const float expected_max_tof = 600.0f / yrt::SPEED_OF_LIGHT_MM_PS;
		REQUIRE(space.getMaxTOF_ps() == Approx(expected_max_tof));
	}
}

TEST_CASE("scatterspace-step-size", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();
	...
	SECTION("10-tof-bins")
	{
		...
		REQUIRE(space.getAngleStep() == Approx(2.0f * M_PI / 30.0f));
		REQUIRE(space.getPlaneStep() == Approx(400.0f / 20.0f));
	}

	SECTION("difference-grid")
	{
		yrt::ScatterSpace space(*scanner, 5, 10, 15);

		REQUIRE(space.getTOFBinStep_ps() ==
		        Approx(space.getMaxTOF_ps() / 5.0f));
		REQUIRE(space.getAngleStep() == Approx(2.0f * M_PI / 15.0f));
		REQUIRE(space.getPlaneStep() == Approx(400.0f / 10.0f));
	}
}

TEST_CASE("scatterspace-index-bidirectionality", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();

	SECTION("tof-bin-tof-value")
	{
		yrt::ScatterSpace space(*scanner, 10, 20, 30);
		...
		{
			float expected_tof = tof_step * (static_cast<float>(bin) + 0.5f);
	...
	SECTION("plane-index-plane-pos")
	{
		...
		{
			float expected_pos =
			    plane_start + plane_step * (static_cast<float>(plane) + 0.5f);
	...
	SECTION("angle-index-angle-value")
	{
		yrt::ScatterSpace space(*scanner, 10, 20, 30);
...
TEST_CASE("scatterspace-wrapangle", "[scatterspace]")
{

	SECTION("between-0-and-2pi")
	{
...
TEST_CASE("scatterspace-clamping", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();
	yrt::ScatterSpace space(*scanner, 10, 20, 30);

	SECTION("clamp-axialfov")
	{
	...
	SECTION("clamp-tof")
	{
		...
		REQUIRE(space.clampTOF(max_tof * 2.0f) == Approx(max_tof - half_step));
	}
}

TEST_CASE("scatterspace-nearestneighbor", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();
		...
		for (size_t p1 = 0; p1 < 20; ++p1)
		{
			for (size_t a1 = 0; a1 < 30; ++a1)
			{
				for (size_t p2 = 0; p2 < 20; ++p2)
				{
					...
					{
						float value = static_cast<float>(t + p1 + a1 + p2 + a2);
	...
	SECTION("nearestneighbor-exact-centers")
	{
		for (size_t t = 0; t < 10; ++t)
		{
	...
	SECTION("nearestneighbor-boundaries")
	{
		float tof_step = space.getTOFBinStep_ps();
		float plane_step = space.getPlaneStep();
		float angle_step = space.getAngleStep();
	...
	SECTION("nearestneighbor-wrapped-angles")
	{
		float angle_step = space.getAngleStep();
		...
		pos.tof_ps = space.getTOF_ps(5);
		pos.planePosition1 = 0.0f;
		pos.angle1 = -angle_step * 0.3f;  // Should wrap to ~2π - 0.3*angle_step
		pos.planePosition2 = 0.0f;
		pos.angle2 = 0.0f;
...
TEST_CASE("scatterspace-linearinterpolation", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();
	yrt::ScatterSpace space(*scanner, 2, 2, 2);  // Small grid for testing

	SECTION("linearinterpolation-exact-centers")
	{
		// Interpolation at exact grid points returns same value
		// TODO NOW: This
	}

	SECTION("linearinterpolation-middle-of-grid")
	{
...
TEST_CASE("scatterspace-edgecases", "[scatterspace]")
{
	auto scanner = yrt::util::test::makeScanner();

	SECTION("minimum-num-bins")
	{
		yrt::ScatterSpace space(*scanner, 1, 1, 2);  // Need at least 2 angles
...
TEST_CASE("scatterspace-random-grid-sizes", "[scatterspace]")
{
...
TEST_CASE("scatterspace-getters-and-setters", "[scatterspace]")
{
